...
from pipelines.mrcnn import utils
...
def detection_targets_graph(proposals, gt_class_ids, gt_boxes, gt_masks, config):
    ...
    masks: [TRAIN_ROIS_PER_IMAGE, height, width). Masks cropped to bbox
           boundaries and re-sized to neural network output size.
...
class DetectionTargetLayer(KE.Layer):
    ...
    target_mask: [batch, TRAIN_ROIS_PER_IMAGE, height, width)
                 Masks cropped to bbox boundaries and re-sized to neural
                 network output size.
...
class DetectionLayer(KE.Layer):
    ...
    def call(self, inputs):
        ...
        # because we know that all images get re-sized to the same size.
...
def mrcnn_class_loss_graph(target_class_ids, pred_class_logits,
    ...
    active_class_ids: [batch, num_classes]. Has a value of 1 for
        classes that are in the data set of the image, and 0
        for classes that are not in the data set.
    """
    ...
    # Find predictions of classes that are not in the data set.
...
def load_image_gt(dataset, config, image_id, augment=False, augmentation=None,
    ...
    mask, class_ids, bb = dataset.load_mask(image_id, with_bb=True)
...
def build_detection_targets(rpn_rois, gt_class_ids, gt_boxes, gt_masks, config):
    ...
    masks: [TRAIN_ROIS_PER_IMAGE, height, width, NUM_CLASSES). Class specific masks cropped
           to bbox boundaries and re-sized to neural network output size.
    """
...
def data_generator(dataset, config, shuffle=True, augment=False, augmentation=None,
    ...
    generator returns two lists, inputs and outputs. The containes
            ...
            if b >= batch_size:
                ...
                if random_rois:
                    ...
                    if detection_targets:
                        ...
                        # Keras requires that output and targets have the same number of
                        # dimensions
...
class MaskRCNN():
    ...
    def set_log_dir(self, model_path=None):
        ...
        self.checkpoint_path = os.path.join(self.log_dir, "mask_rcnn_*epoch*.h5")
    ...
    def mold_inputs(self, images):
        ...
        molded_images: [N, h, w, 3]. Images re-sized and normalized.
    ...
    def unmold_detections(self, detections, mrcnn_mask, original_image_shape,
        ...
        # Translate normalized coordinates in the re-sized image to pixel
...
def compose_image_meta(image_id, original_image_shape, image_shape,
    ...
    active_class_ids: List of class_ids available in the data set from which
        the image came. Useful if training on images from multiple data sets
        where not all classes are present in all data sets.
    """
    meta = np.array(
        [image_id] +  # size=1
...
def mold_image(images, config):
    """
    Expects an RGB image (or array of images) and sub-traces
